...
const unitFormSchema = z.object({
  unitName: z
    .string()
    .min(12, { message: 'unit name must be at least 12 characters' }),
  ...
  const handleSubmit = async (data: z.infer<typeof unitFormSchema>) => {
    ...
    try {
      const newunit = await createUnit(
        ...
        state.section.selectedSectionId
      )
        ...
        <DrawerContent>
          <div className='min-h-[300px] w-[600px] mx-auto py-10'>
            <h1 className='text-lg font-medium mb-4 font-display'>
              Unit for{' '}
              {
                state.section.list.find(
                  section => section.id === state.section.selectedSectionId
                )?.title
              }
            </h1>
            <Form {...form}>
              <form
                ...
                <FormItem>
                  ...
                  <Textarea
                    id='unitDescription'
                    {...form.register('unitDescription')}
                  />
                  <FormMessage>
                    {form.formState.errors.unitDescription?.message}
                  </FormMessage>
                </FormItem>
