...
export default function Page() {
  ...
  const [selectedLanguage, setSelectedLanguage] = useState<keyof typeof languages | null>(null)
    ...
    <div>
      ...
      <div className='mt-8'>
        {isCreating ? (
          <div>
            <div>
              <Button onClick={() => setIsCreating(false)} theme='secondary' className='mb-4'>
                Back
              </Button>
            </div>
            <div className='flex flex-col justify-center gap-4 max-w-[200px] mx-auto'>
              <SelectCourse setSelectedLanguage={setSelectedLanguage} selectedLang={selectedLanguage} />
              <Button
                onClick={saveCourse}
                theme={'secondary'}
                className='w-full'
          ...
          <div className='grid grid-cols-5 grid-rows-[repeat(auto-fill,200px)] gap-6'>
            <Card
              ...
              <div className='space-y-2'>
                ...
                <h3 className='text-lg font-display text-neutral-600'>Add course</h3>
              </div>
            </Card>
            {courses &&
              courses.map(course => (
                <Card
                  ...
                  <h3 className='text-lg font-display text-neutral-600'>{course.name}</h3>
                            ...
                            return (
                              <div className='p-4 border border-slate-200 rounded-lg bg-white shadow-[0_6px_20px_1px_rgba(0,0,0,.1)]'>
                                ...
                                <p className='text-sm mb-3'>
                                  All the data associated with this course will be deleted including sections, units,
            ...
            {fetchingCourses && (
              <>
                <Card className='animate-pulse flex items-center flex-col py-8 space-y-4' theme='default'>
...
const SelectCourse = ({
  ...
  setSelectedLanguage: React.Dispatch<React.SetStateAction<keyof typeof languages | null>>
      ...
      <div className='relative'>
        <Card
          ...
          <h3 className='text-lg font-display text-neutral-600'>
            {selectedLang ? languages.get(selectedLang as string) : 'Select language'}
          </h3>
        </Card>
              ...
              {Array.from(languages).map(([key, value]) => (
                <li
                  ...
                  <svg className='h-[35px] w-[45px]'>
                    <use xlinkHref={`/svg/flags.svg#${key}`} className='scale-[.58]' />
                  </svg>
                  <span>{value}</span>
                </li>
              ))}
